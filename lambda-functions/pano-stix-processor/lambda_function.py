# lambda_function.py
"""
STIX Processor Lambda Function for Panorama

Layers Required:
1. panorama-dependencies 
2. panorama-datamodel 
"""
...
from panorama_datamodel.repositories import MitreRepository
from panorama_datamodel.utils import utc_now
...
class STIXProcessor:
    ...
    def _process_technique(self, stix_obj: Dict[str, Any], repo: MitreRepository, memory_store: MemoryStore, domain: str) -> str:
        """Process MITRE technique STIX object with deprecated status tracking"""
        external_refs = stix_obj.get('external_references', [])
        mitre_ref = next((ref for ref in external_refs if ref.get('source_name') == 'mitre-attack'), None)
        ...
        # Extract deprecated and version information
        ...
        # Handle deprecated date - if deprecated but no date, use modified time
        deprecated_date = None
        if is_deprecated or is_revoked:
            deprecated_date = stix_obj.get('x_mitre_deprecated_date')
            if not deprecated_date:
                deprecated_date = stix_obj.get('modified')
            if deprecated_date:
                # Parse ISO format date string
                from datetime import datetime
                deprecated_date = datetime.fromisoformat(deprecated_date.replace('Z', '+00:00'))
        
        # Extract superseding technique if available
        superseded_by = None
        deprecation_reason = None
        if is_deprecated:
            # Look for superseding technique in the description or notes
            description = stix_obj.get('description', '')
            if 'superseded by' in description.lower():
                # Extract technique ID from description (e.g., "Superseded by T1234")
                import re
                match = re.search(r'superseded by\s+(T\d{4}(?:\.\d{3})?)', description, re.IGNORECASE)
                if match:
                    superseded_by = match.group(1)
            
            # Store deprecation reason if available
            deprecation_reason = stix_obj.get('x_mitre_deprecation_reason')
            if not deprecation_reason and (is_deprecated or is_revoked):
                deprecation_reason = 'Revoked by MITRE' if is_revoked else 'Deprecated by MITRE'
        ...
        if existing_technique:
            ...
            if existing_technique.is_deprecated != is_deprecated:
                logger.info(f"Technique {technique_id} deprecation status changed: {existing_technique.is_deprecated} -> {is_deprecated}")
            
            logger.info(f"Updated technique: {technique_id} (deprecated: {is_deprecated}, revoked: {is_revoked})")
            return 'updated'
        else:
            # Create new technique
            new_technique = MitreTechnique(**technique_data)
            repo.session.add(new_technique)
            logger.info(f"Created technique: {technique_id} (deprecated: {is_deprecated}, revoked: {is_revoked})")
            return 'created'
